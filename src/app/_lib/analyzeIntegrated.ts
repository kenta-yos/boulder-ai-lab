...
async function readVideoNotes(
  ...
  const prompt =
    ...
    "- 出力は簡潔に。動画全体（スタート〜落下/完登）を、意味のある局面ごとに1行で並べた時系列にする。目安は合計15〜40行。0.25秒ごとの細かな列挙や、同じ状態の言い換え反復はしない。ムーブの切れ目・持ち替え・体勢の崩れ・落下など『変化した瞬間』を秒数つきで拾う。最後まで書き切ることを最優先にする（詳しさより全体をカバーすることを優先）。\n" +
    "- ただし落下（または完登）に至るラスト約10秒は最重要。ここだけは細かく丁寧に読む：崩れ始めた瞬間から落ちる（または取り切る）まで、手・足・腰・重心が秒ごとにどう動いたか、そしてどこがどう破綻したか（どの手/足が離れたか、体がどちらに回ったか、どのホールドで止まれなかったか等）を、見えた範囲で具体的に。序盤〜中盤は簡潔でよいが、この区間は密に書く。\n" +
    "- 体の使い方を淡々と。次は『観察の視点』（毎回すべてを書くのではなく、その局面で目立った点だけ拾う）。良し悪しや技術の正否は判断せず、見えた事実だけを述べる（助言・講評はしない）：\n" +
...
出力（JSON）：
- findings（秒数つきの指摘・これが主役）：tSec（秒・数値）と comment（その場のコーチング）で3〜6個、時間順に並べる。主軸は「落ちた（または取り切れなかった）瞬間のムーブ」。まず落下の核心について、崩れ始めから落ちるまでを複数の指摘で厚く掘り下げる（ここが最重要）。そのうえで、序盤〜中盤に明確な課題があれば併せて出す（無ければ無理に作らない）。各 comment は必ず「その瞬間のムーブの何が問題か（体のどこがどうなっているか）」と「どう直すか（体のどこを・どの順で・どう動かすか。知識ベースの技術名・合図つき）」の両方を、一言で終わらせずしっかり解説する（目安2〜4文）。関連する技術軸があれば skill も付ける。tSec の付け方：静止画に映っている範囲（動画末尾＝落下の核心）は、ユーザーメッセージで渡す『静止画の正確な時刻』から、その瞬間が映っているコマの秒数を使う。静止画に映らない序盤〜中盤だけ動きメモの秒数を使う（動きメモの秒数はズレることがあるため、静止画で確認できる所は静止画の時刻を優先）。
- summary（総括）：最後に、全体を通したアドバイスを2〜5文。個々の指摘の寄せ集めではなく、この登りで一番効く改善の軸を一つ立て、次のトライで何を意識すべきかを述べる。休憩中に数秒で読める簡潔さ。
...
async function coach(
  ...
  const userText =
    ...
    (trend
      ...
      : "") +
    "\n\n知識ベースに基づき、秒数つきの指摘（落ちた瞬間のムーブを主役に厚く。途中の課題もあれば）・総括・8軸スコアを出してください。";

  const response = await client.messages.create({
    model: CLAUDE_MODEL,
    // 指摘を厚く（各2〜4文×3〜6個）＋総括＋8軸ぶんの余裕を持たせる。
    max_tokens: 4000,
