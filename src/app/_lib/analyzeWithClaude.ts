...
- 専門用語には一言の補足を添えてください。
- 左右（右手/左手・右足/左足）は画像から取り違えやすい。まず登り手がカメラに対して後ろ向き（背中側）か前向きかを見極め、左右は必ず登り手自身の体を基準に述べてください（見る人から見た左右ではありません）。向きに自信が持てないときは左右の断定を避け、ホールドの色や位置、壁側/外側の手足、上/下の手足など取り違えにくい言い方で指摘してください。
